    ...
    while true do
        ...
        if c:connect(addr, port) then
            ...
            while true do
                ...
                if result then
                    sys.publish("CHANNEL_STATUS",(passon and cid or uid),1)
                    -- 这里执行用户自定义的指令
        ...
        c:close()
        datalink = false
        sys.publish("CHANNEL_STATUS",(passon and cid or uid),0)
        sys.wait((2 ^ idx) * 1000)
...
local function mqttTask(cid, pios, reg, convert, passon, upprot, dwprot, keepAlive, timeout, addr, port, usr, pwd, cleansession, sub, pub, qos, retain, uid, clientID, addImei, ssl, will, cert)
    ...
    while true do
        ...
        if mqttc:connect(addr, port, ssl == "tcp_ssl" and ssl or nil, cert) then
            -- 初始化订阅主题
...
local function oneNet_mqtt(cid, pios, reg, convert, passon, upprot, dwprot, keepAlive, timeout, addr, port, regcode, pid, ptype, cleansession, qos, retain, uid)
    ...
    while true do
        ...
        if mqttc:connect(addr, port) then

...
elseif is8910 then
    ...
    pios = {
        ...
        pio1 = pins.setup(1, nil, pio.PULLDOWN),
        pio2 = pins.setup(2, nil, pio.PULLDOWN),
        pio4 = pins.setup(4, nil, pio.PULLDOWN),
        ...
        --modified by miuser 2021-08-10
        --为了兼容sd卡，读写，此处初始化后就不能读写了
        --pio24 = pins.setup(24, nil, pio.PULLDOWN),
        --pio25 = pins.setup(25, nil, pio.PULLDOWN),
        --pio26 = pins.setup(26, nil, pio.PULLDOWN),
        --pio27 = pins.setup(27, nil, pio.PULLDOWN),
...
if not dtu.pins or not dtu.pins[3] or not pios[dtu.pins[3]] then -- 这么定义是为了和之前的代码兼容
    if is8910 then
        rspin = 3
        pins.setup(3, resetConfig, pio.PULLUP)
    elseif is1802S then
        pins.setup(63, resetConfig, pio.PULLUP)
        pins.setup(17, resetConfig, pio.PULLUP)
    elseif is4gLod then
        pins.setup(63, resetConfig, pio.PULLUP)
        pins.setup(68, resetConfig, pio.PULLUP)
    else
        pins.setup(29, resetConfig, pio.PULLUP)
    end

else
    pins.setup(tonumber(dtu.pins[3]:sub(4, -1)), resetConfig, pio.PULLUP)
    pios[dtu.pins[3]] = nil
end
-- NETLED指示灯任务
